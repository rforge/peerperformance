####################################################################################
## Set of R functions for the optimal block length computation for the modified Sharpe ratio
####################################################################################

.msharpeBlockSize = function(x, y, level = 0.90, na.neg = TRUE, control = list(), 
  ...
  d = msharpe.ratio.diff(x, y, rf.x = 0, rf.y = 0, level, na.neg, ctr$ttype)
  T = length(x)
  ...
  for (k in 1 : K){
    ...
    for (t in 2:(T.start + T)){
      ...
      Var.data[t,2] = coef2[1] + coef2[2] * Var.data[t-1, 2] + coef2[3] * Var.data[t-1, 2] + resid.mat.star[t, 2]
    }
    Var.data.trunc = Var.data[(T.start + 1):(T.start + T),]
    for (j in 1 : b.len){
      bsids = bootIndices(T, M, b.vec[j])
      tmp   = msharpeTestBootstrap(Var.data.trunc, rf.x = 0, rf.y = 0, level, na.neg, bsids, b.vec[j], ctr$ttype, pBoot = 1, d)
      if (tmp$pval <= alpha){
